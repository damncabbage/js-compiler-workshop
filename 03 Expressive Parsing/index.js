...
function tokensToAst(inputTokens) {
  ...
  // EXERCISE:
  // Have a read of the below parseExpression() function, and how
  // it uses peekIs() to check what's coming, committing to
  // parsing (with consume()) once it's had a look.
  //
  // Then duck on down to parseFunctionCall after it.
  ...
  function parseFunctionCall() {
    ...
    if (!peekIs("parenClose", 1)) {
      ...
      // Let's go and process that now...
      args.push(parseExpression());

      // EXERCISE:
      // Figure out how to consume the rest of the arguments,
